...
class MaYiWenKu(object):
    ...
    def images2pdf(self, image_dir):
        ...
        for image_name in image_list:
            ...
            # 处理图片
            image_real_path = self.cut_image(image_real_path, (int(a4_width * 0.95), int(a4_height * 0.95)))
    ...
    def get_document(self, url, max_page=None):
        ...
        for page_index in range(1, max_page + 1):
            image_path = os.path.join(image_dir, "{}.png".format(page_index))
            if not os.path.exists(image_path):
                image_content = self.download_image("{}{}.gif".format(image_base_url, page_index), url)
                with open(image_path, "wb") as f:
                    f.write(image_content)
                self.log("\t第{}页下载成功".format(page_index))
                time.sleep(1)
            else:
